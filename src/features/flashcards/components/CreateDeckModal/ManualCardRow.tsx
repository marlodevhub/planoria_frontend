import type { ManualCard } from '../../types/flashcard.types'

interface ManualCardRowProps {
    card: ManualCard
    index: number
    onChange: (id: string, field: 'question' | 'answer', value: string) => void
    onDelete: (id: string) => void
    canDelete: boolean
}

const inputCls = [
    'w-full bg-bg border border-border rounded-lg px-3 py-2',
    'text-text text-sm placeholder:text-muted',
    'focus:outline-none focus:border-accent/60 transition-colors resize-none',
].join(' ')

export function ManualCardRow({ card, index, onChange, onDelete, canDelete }: ManualCardRowProps) {
    return (
        <div className="flex items-start gap-3">
            <span className="text-muted text-xs font-mono pt-2.5 w-5 flex-shrink-0 text-right">
                {index + 1}
            </span>
            <div className="flex-1 space-y-2">
                <textarea
                    value={card.question}
                    onChange={(e) => onChange(card.id, 'question', e.target.value)}
                    placeholder="Pregunta..."
                    rows={2}
                    className={inputCls}
                />
                <textarea
                    value={card.answer}
                    onChange={(e) => onChange(card.id, 'answer', e.target.value)}
                    placeholder="Respuesta..."
                    rows={2}
                    className={inputCls}
                />
            </div>
            <button
                type="button"
                onClick={() => onDelete(card.id)}
                disabled={!canDelete}
                className="mt-1.5 h-8 w-8 flex items-center justify-center rounded-lg border border-border text-muted hover:border-red-500/50 hover:text-red-400 hover:bg-red-500/5 transition-all disabled:opacity-30 disabled:cursor-not-allowed flex-shrink-0"
                aria-label="Eliminar tarjeta"
            >
                <i className="ti ti-trash text-[15px]" aria-hidden="true" />
            </button>
        </div>
    )
}