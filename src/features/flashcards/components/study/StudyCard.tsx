...
export function StudyCard({ card, flipped, cardsBehind, onFlip, onResetFlip, onNext }: StudyCardProps) {
    ...
    return (
        <div className="relative w-80 h-52" ref={wrapperRef}>
            {cardsBehind >= 2 && (
                <div className="absolute inset-0 rounded-2xl bg-surface border border-border translate-y-6 scale-[0.94] opacity-30" />
            )}
            {cardsBehind >= 1 && (
                <div className="absolute inset-0 rounded-2xl bg-surface border border-border translate-y-3 scale-[0.97] opacity-60" />
            )}
            ...
            >
                <div ref={innerRef} className="w-full h-full relative" style={{ transformStyle: 'preserve-3d' }}>
                    {/* Frente */}
                    <div
                        className="absolute inset-0 rounded-2xl bg-surface border border-border flex flex-col items-center justify-center p-6 text-center"
                        style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}
                    >
                        <span className="text-muted text-[11px] font-mono uppercase tracking-widest mb-3">Pregunta</span>
                        <p className="text-text font-semibold text-lg leading-snug">{card.question}</p>
