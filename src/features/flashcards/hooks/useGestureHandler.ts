...
const DRAG_THRESHOLD_PX = 160;
const DRAG_THRESHOLD_FAST = 55;
...
export function useGestureHandler({
  ...
  useEffect(() => {
    onResetFlipRef.current = onResetFlip;
  }, [onResetFlip]);
  useEffect(() => {
  ...
  const exitAndAdvance = useCallback(
    (wasKnown: boolean) => {
      const el = innerRef.current;
      if (!el) {
  ...
  const onPointerMove = useCallback(
    (clientY: number) => {
      ...
      if (dt > 0) velocityY.current = (clientY - lastY.current) / dt;
      lastY.current = clientY;
      ...
      if (gesture.current === "dragging") applyDrag(dy);
    },
