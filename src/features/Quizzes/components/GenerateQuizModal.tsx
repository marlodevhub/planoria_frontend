      ...
      >
        {file ? (
          <>
            ...
            <p className="text-xs text-muted-foreground">Click para cambiar archivo</p>
          </>
        ) : (
          <>
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-foreground">Subí tu archivo PDF</p>
              <p className="text-xs text-muted-foreground mt-0.5">Click para seleccionar</p>
            </div>
          </>
        )}
        <input
...
export function GenerateQuizModal({ open, onClose }: GenerateQuizModalProps) {
  ...
  const onStep2Submit = async (data: Step2Fields) => {
    ...
    setIsUploading(true)
    try {
      const uploaded = await quizService.uploadFile(Number(step1Data.courseId), step1Data.file)
      setIsUploading(false)
      setIsGenerating(true)
      await quizService.generateFromPdf({
        fileId: uploaded.id,
        topic: data.topic,
        ...
        <div className="px-6 pt-5 pb-4 border-b border-border">
          <DialogHeader>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-muted-foreground font-mono">{step}/2</span>
            </div>
            <DialogTitle className="text-base">
              {step === 1 ? 'Seleccionar curso y archivo' : 'Configurar generación'}
            </DialogTitle>
            <DialogDescription className="text-xs">
              {step === 1
                ? 'Elegí el curso y subí el PDF fuente'
                : 'Configurá cómo querés generar el quiz'}
            </DialogDescription>
          </DialogHeader>
        </div>
