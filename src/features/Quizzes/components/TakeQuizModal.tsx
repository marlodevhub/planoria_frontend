...
export function TakeQuizModal({ quiz, onClose }: TakeQuizModalProps) {
  ...
  const [timeLeft, setTimeLeft] = useState(quiz.duracionMinutos * 60)
  ...
  const handleStart = useCallback(async () => {
    setError(null)
    setTimeLeft(quiz.duracionMinutos * 60)
    try {
  ...
  }, [quiz.id, quiz.duracionMinutos, startAttempt])

  useEffect(() => {
    if (phase !== 'taking' || timeLeft <= 0 || !quiz.duracionMinutos) return
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="rounded-xl bg-muted/50 p-3 flex flex-col gap-1">
                ...
                <p className="text-sm font-semibold text-foreground">
                  {quiz.duracionMinutos > 0
                    ? `${quiz.duracionMinutos} min`
                    : 'Sin límite'}
                </p>
              </div>
            </div>
          ...
          <Card className="w-full p-6 space-y-5 border-0 shadow-none">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                {quiz.duracionMinutos > 0 && timeLeft > 0 && (
                  <span className={`font-mono ${timeLeft <= 60 ? 'text-destructive font-semibold' : ''}`}>
