...
export function QuizAttemptHistoryModal({
  ...
  const { data: history, isLoading: historyLoading, isError: historyError } = useAttemptHistory(quizId);
  const { data: best, isLoading: bestLoading, isError: bestError } = useBestAttempt(quizId);
