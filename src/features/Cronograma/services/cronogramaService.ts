...
export const cronogramaService = {
   ...
   */
  async getContents(scheduleId: number): Promise<ScheduleContentItem[]> {
    const { data } = await api.get<ScheduleContentItem[]>(
  ...
  async deleteContent(scheduleId: number, contentId: number): Promise<void> {
    await api.delete(CRONOGRAMA_API_ROUTES.CONTENTS(scheduleId), {
      params: { contentId },
    })
  },
   ...
   */
  async autoAssignContent(scheduleId: number): Promise<{ assignedCount: number }> {
    const { data } = await api.post<{ assignedCount: number }>(CRONOGRAMA_API_ROUTES.AUTO_ASSIGN(scheduleId))
    return data
  },
