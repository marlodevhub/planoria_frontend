...
const contentSchema = z.object({
  contentType: z.enum(['flashcard_deck', 'quiz']),
...
export function CronogramaPage() {
  ...
  const [selectedScheduleCourseName, setSelectedScheduleCourseName] = useState<string | null>(null)
  ...
  function openContentModal(scheduleId: number) {
    ...
    setSelectedScheduleCourseName(schedule?.courseName ?? null)
  ...
  function toggleScheduleContents(scheduleId: number) {
  ...
  function onContentSubmit(data: ContentFormFields) {
    ...
    createContent.mutate(
      ...
      },
      {
        ...
        onError: (err: any) => {
          setContentError(err?.response?.data?.message || 'Error al asignar contenido')
        },
      },
    )
  }
  ...
  async function handleAutoAssign() {
    ...
    try {
      ...
      if (result.assignedCount === 0) {
        setContentError('No hay cursos vinculados o contenido disponible para auto-asignar')
      } else {
        setShowContentModal(false)
      }
    } catch (err: any) {
      setContentError(err?.response?.data?.message || 'Error al auto-asignar contenido')
    }
  }
  ...
  const availableFlashcards = (decks ?? []).filter(
    (d) => !selectedScheduleCourseName || d.courseName === selectedScheduleCourseName
  )
  const availableQuizzes = (quizzes?.filter((q) => q.activo) ?? []).filter(
    (q) => !selectedScheduleCourseName || q.cursoNombre === selectedScheduleCourseName
  )
                ...
                {daySchedules.map((sched) => (
                  <div key={sched.id} className="rounded-xl border border-border p-3 space-y-1">
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      <button
                        onClick={() => toggleScheduleContents(sched.id)}
                        className="text-xs text-muted-foreground hover:text-foreground transition-colors"
                      >
                        {selectedScheduleId === sched.id ? '▼ Ocultar' : '▶ Contenido'}
                      </button>
                      <button
                    ...
                    {selectedScheduleId === sched.id && contents && Array.isArray(contents) && (
                      <div className="mt-3 border-t border-border pt-3">
                        {contents.length === 0 ? (
                          <div className="text-center py-2">
                            <p className="text-xs text-muted-foreground">Sin contenido asignado</p>
                            <p className="text-[10px] text-muted-foreground/60 mt-0.5">Usá el botón + para agregar quizzes o flashcards</p>
                          </div>
                        ) : (
                          <div className="space-y-2">
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-medium text-muted-foreground">
                                {contents.length} contenido{contents.length !== 1 ? 's' : ''} · {contents.filter(c => c.completed).length} completado{contents.filter(c => c.completed).length !== 1 ? 's' : ''}
                              </span>
                              <span className="text-[10px] text-muted-foreground">
                                {contents.reduce((t, c) => t + (c.estimatedMinutes || 0), 0)} min total
                              </span>
                            </div>
                            {contents.map((c) => (
                              <div
                                key={c.id}
                                className="flex items-center justify-between bg-muted/30 rounded-lg px-2.5 py-2 cursor-pointer hover:bg-muted/50 transition-colors"
                                onClick={() => {
                                  if (c.contentType === 'quiz') {
                                    navigate(ROUTES.QUIZZES, { state: { openQuizId: c.contentId } })
                                  } else {
                                    navigate(buildRoute.deck(c.contentId))
                                  }
                                }}
                              >
                                <div className="flex items-center gap-2 min-w-0">
                                  <div className={`shrink-0 w-6 h-6 rounded-md flex items-center justify-center text-[11px] ${c.completed ? 'bg-green-500/20 text-green-600' : 'bg-accent/20 text-accent'}`}>
                                    {c.contentType === 'quiz' ? '📝' : '🃏'}
                                  </div>
                                  <div className="min-w-0">
                                    <p className="text-xs font-medium text-foreground truncate max-w-[140px]">{c.contentName || (c.contentType === 'quiz' ? 'Quiz' : 'Flashcards')}</p>
                                    <p className="text-[10px] text-muted-foreground">
                                      {c.contentType === 'quiz' ? 'Quiz' : 'Flashcards'} · {c.estimatedMinutes}min
                                      {c.completed && ' · Completado'}
                                    </p>
                                  </div>
                                </div>
                                <button
                                  onClick={(e) => { e.stopPropagation(); deleteContent.mutate(c.id) }}
        ...
        <DialogContent className="max-w-md">
          ...
          <Form {...contentForm}>
            <form onSubmit={contentForm.handleSubmit(onContentSubmit)} className="space-y-4">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="flashcard_deck">Flashcard</SelectItem>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {contentForm.watch('contentType') === 'flashcard_deck'
              ...
              {contentError && (
                <p className="text-xs text-destructive bg-destructive/10 px-3 py-2 rounded-lg">{contentError}</p>
              )}
              <div className="flex gap-3 pt-2">
